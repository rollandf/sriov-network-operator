...
// from a managed bridge. The function detach interface from managed bridges only.
func (b *bridge) DetachInterfaceFromManagedBridge(pciAddr string, pfName string, numVfs int) error {
	log.Log.V(1).Info("DetachInterfaceFromManagedBridge(): detach interface", "pciAddr", pciAddr, "pfName", pfName, "numVfs", numVfs)
	if err := b.ovs.RemoveInterfaceFromOVSBridge(context.Background(), pciAddr, pfName, numVfs); err != nil {
